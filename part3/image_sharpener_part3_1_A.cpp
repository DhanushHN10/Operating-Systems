...
void S2_find_details(struct image_t *input_image, void *S1_S2_ptr, sem_t *S1_S2_sem, sem_t *S2_S1_sem, int client_fd)
{
    int rows_read = 0;
    ...
    while (1)
    {
        ...
        memcpy(p, &row_index, sizeof(int));
        memcpy(p + sizeof(int), out_row, sizeof(pixel) * input_image->width);
...
int main(int argc, char **argv)
{
    ...
    *((int *)S1_S2) = -1;
    ...
    size[0] = input_image->width;
    size[1] = input_image->height;
    send_all(client_fd, (char *)size, sizeof(int) * 2);
    ...
    else
    {
        ...
        for (int i = 0; i < noi; i++)
        {
            S2_find_details(input_image, S1_S2, S1_S2_sem, S2_S1_sem, client_fd);
