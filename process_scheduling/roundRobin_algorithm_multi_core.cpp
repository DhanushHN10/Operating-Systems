...
void roundRobin(queue<Process*>& process_queue, Process*& curr_process,set<Process*>& waiting_processes, ll& timer, int& time_quanta, vector<vector<int>>& gantt_chart, int cpu_no)
{
...
bool all_idle(const vector<Process*>& curr_processes) {
    for (const auto* proc : curr_processes) {
        if (proc != nullptr) {
            return false; 
        }
    }
...
int main(int argc, char* argv[])
{
    ...
    vector<Process*> curr_processes(no_cpus,nullptr);   // running on the single core cpu
    ...
    while(!process_queue.empty() || !waiting_processes.empty() || !all_idle(curr_processes) ||  process_idx < all_processes.size())
    {
        ...
        {
        roundRobin(process_queue,curr_processes[i],waiting_processes,timer,time_quanta,gantt_chart, i);
...
ofstream gantt_file("gantt_chart_RR_multi_core.txt");
