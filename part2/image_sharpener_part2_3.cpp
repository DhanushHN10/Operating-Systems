...
void S1_smoothen(struct image_t *input_image, void *S1_S2_ptr, sem_t *S1_S2_sem)
{
    // TODO
...
void S2_find_details(struct image_t *input_image, void *S1_S2_ptr, void *S2_S3_ptr, sem_t *S1_S2_sem, sem_t *S2_S3_sem)
{
    int rows_read = 0;
...
void S3_sharpen(struct image_t *input_image, void *S2_S3_ptr, sem_t *S2_S3_sem)
{
    // TODO
    int rows_read = 0;
    pixel *row = new pixel[input_image->width];
...
int main(int argc, char **argv)
{
    ...
    sem_t *S1_S2_sem = sem_open("/S1_S2_sem", O_CREAT | O_EXCL, 0666, 1);
    sem_t *S2_S3_sem = sem_open("/S2_S3_sem", O_CREAT | O_EXCL, 0666, 1);
    ...
    if (S1_pid == 0)
    {
        ...
        for (int i = 0; i < noi; i++)
        {
            S1_smoothen(input_image, S1_S2, S1_S2_sem);
    ...
    else
    {
        ...
        if (S2_pid == 0)
        {
            ...
            for (int i = 0; i < noi; i++)
            {
                S2_find_details(input_image, S1_S2, S2_S3, S1_S2_sem, S2_S3_sem);
        ...
        else
        {
            ...
            for (int i = 0; i < noi; i++)
            {
                S3_sharpen(input_image, S2_S3, S2_S3_sem);
                cout
